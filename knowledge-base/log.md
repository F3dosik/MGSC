...
## [2026-05-22] meta | Глава 7 написана; бенчмарк random; план гл. 7 пересобран

Текстовая работа:
- Переписана структура главы 7 «Экспериментальные результаты»: 7 подразделов (Среда / Валидация / Random baseline / Одиночный SA / Мультистарт / Структура ландшафта / Сводный анализ) + ВЫВОДЫ. ~7 страниц.
- Главу 7 формирует три тезиса: (T1) корректность инструмента, (T2) SA даёт NL=100 и два бассейна изолированы, (T3) CJS-функция и параметры MDPI 2022 не работают для биективных 8×8.
- Кульминация — 7.6 «Структура ландшафта вокруг эталонной точки» — объединяет E-07 (HC из NL=100, 0 улучшений) и E-10 (SA из AES, 16/16 остались на AES, 64M swap-попыток без перехода).
- 7.5 — короткий подраздел про нулевую дисперсию мультистарта (E-08), теперь подаётся как структурный, а не статистический эффект.
- Переработанные подразделы 7.1, 7.2 (с особенностью SM4 — 1 fixed point), 7.3 (с гистограммами random baseline).
- Стиль согласован с правилами антиплагиата: без жирных текстовых маркеров абзацев, чередование длин предложений, разрушенные параллелизмы, конкретные числа из E-NN.

Бенчмарк random:
- Реализован `research/random_benchmark/main.go` — параллельный сбор метрик для N случайных биективных подстановок через Fisher–Yates с детерминированным seed.
- Прогон на 1000 подстановках (4-ядерный, ~1 минута): NL min=84, med=94, max=98; δ min=10, med=12, max=18; deg=7, AI=3 на всех; 36.0% без фикс. точек (≈ теоретическое 1/e); 0/1000 под всеми тремя профилями verify.
- Создан `research/random_benchmark/plot.py` — сводка процентилей + гистограммы 2×2 (NL, δ, deg, AI) в PNG. Используется существующий `research/venv/` с pandas+matplotlib.
- CSV и PNG сохранены, PNG скопирован в `report/Images/` для подключения через `\includegraphics` в гл. 7.3.

База знаний:
- Создана [[random-sboxes-baseline]] — сводка распределения метрик случайной выборки, теоретическое подтверждение $1/e$ для неподвижных точек, материал для гл. 7.3.

Закрыто в [[backlog]]: бенчмарк 1000 случайных S-блоков; сравнительная таблица random/SA/эталоны (теперь это таблица в гл. 7.7).
Следующий шаг (зафиксирован в backlog): написание ЗАКЛЮЧЕНИЯ работы.
